...
| P3b | **Setor é opcional no funcionário**, divergindo do modelo original que o exigia | Firme | Exigir taxonomia antes do primeiro cadastro é atrito no momento em que o cliente avalia o produto. Ele organiza quando o setor passa a servir para algo. [Detalhe](produto/modelo-de-dominio.md#people) |
| P3c | **Cargo vira entidade** (era texto livre), com nome dobrado e único por tenant | Firme | "Quantos vigilantes cobrem este turno?" (Marco 4) não tem resposta sobre texto onde "Vigilante" e "vigilante " são dois cargos. A importação cria o que faltar, para não reintroduzir atrito. |
| P12 | **Regime de vínculo é do CONTRATO** (CLT, estatutário, militar, outro), com política de férias e jornada resolvida por regime | Firme | Prefeitura tem estatutário e celetista na mesma folha. Aplicar regra de CLT a militar e chamar de conformidade seria pior que não validar. [ADR-0017](adr/0017-regime-de-vinculo.md) |
| P13 | **Empresa independente = tenant. Subordinada = unidade da árvore** dentro do tenant. RLS nunca vira hierárquica | Firme | A RLS é confiável porque é burra: igualdade simples, sem travessia. Trocar por subárvore transforma todo erro de travessia em vazamento entre clientes. [ADR-0018](adr/0018-organizacoes-e-unidades.md) |
| P14 | **Escala é rodízio justo por serviço**, não ciclo fixo de turno. Ciclo vira um dos modos | Firme | "20 pessoas para a marcha de sábado, de quem é a vez" é o caso real. Mais barato que gerador de ciclo. [ADR-0019](adr/0019-escala-por-rodizio.md) |
| P15 | A escala é **proposta explicável**, com desempate estável — nunca sorteio | Firme | "Por que o Silva e não eu?" precisa ter resposta em uma frase, senão o sargento volta para a planilha na primeira reclamação. [ADR-0019](adr/0019-escala-por-rodizio.md) |
...
| S6 | ~~`User` global por e-mail + `Membership`~~ → **`User` pertence à empresa**, com papel próprio. `Membership` deixa de existir | Substituída | A [ADR-0018](adr/0018-organizacoes-e-unidades.md) derrubou o caso do grupo, e com piloto militar "descobrir que este e-mail existe em outra unidade" vira enumeração de efetivo. [ADR-0020](adr/0020-usuario-pertence-a-empresa.md) |
| S6c | Mesma pessoa em duas empresas: **segundo endereço agora**; desambiguação depois da senha só quando um cliente pedir | Firme | Ir de único-global para único-por-tenant é trocar um índice, sem migração nem fusão de conta. O arrependimento é barato, então hoje vale a decisão mais restrita. [ADR-0020](adr/0020-usuario-pertence-a-empresa.md#mesma-pessoa) |
| S6b | **E-mail continua único no sistema inteiro**, mesmo com o usuário pertencendo a uma empresa | Firme | Único global ≠ usuário global. É o que mantém o login com e-mail e senha só, sem subdomínio nem campo de empresa, e sem customizar o Identity. [ADR-0020](adr/0020-usuario-pertence-a-empresa.md) |
